                                ...
                                struct vn_command_vkBindImageMemory2 *args)
{
   ...
   if (args->ret == VK_SUCCESS) {
      ...
      if (pfn_getMTLTexture) {
         ...
         /* egg_virgl_get_mtl_buffer_contents: ObjC helper that calls [MTLBuffer contents] */
         extern void *egg_virgl_get_mtl_buffer_contents(void *mtl_buffer);
         extern void egg_virgl_track_metal_texture_with_memory(
         ...
         for (uint32_t i = 0; i < args->bindInfoCount; i++) {
            ...
            if (mtl_texture) {
               ...
               if (pfn_getMTLBuffer) {
                  ...
                  if (mtl_buffer) {
                     mtl_buffer_ptr = egg_virgl_get_mtl_buffer_contents(mtl_buffer);
                  }
               }
               egg_virgl_track_metal_texture_with_memory(
